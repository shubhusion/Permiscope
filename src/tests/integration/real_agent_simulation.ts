...
class AutonomousAgent {
    ...
    async executeStep(stepName: string, action: Action, options: { dryRun?: boolean, shadowMode?: boolean } = {}) {
...
async function runScenarios() {
    ...
    await agent.executeStep('Read Secret (Dry Run)', {
        actionName: 'read_file',
    ...
    await agent.executeStep('Plan Dangerous Move (Dry Run)', {
        actionName: 'run_command',
        parameters: { command: 'rm -rf /' },
        agentId: '', timestamp: new Date()
    }, { dryRun: true });
    ...
    const shadowResult = await agent.executeStep('Try Malware (Shadow Mode)', {
        actionName: 'run_command',
        parameters: { command: 'rm -rf /' },
        agentId: '', timestamp: new Date()
    }, { shadowMode: true });
